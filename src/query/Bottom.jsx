...
const Filter = memo(function Filter(props) { // 筛选列表自组件
...
const Option = memo(function Option(props) { // 筛选模块大列表
...
const BottomModal = memo(function BottomModal(props) { // 综合筛选图层
  const {
    ticketTypes,
    trainTypes,
    departStations,
  ...
  const [
    ...
    checkedReducer, 
    checkedTicketTypes, 
    checkedTicketTypes => { // 不传函数为同步，传入函数为异步
    return {
      ...checkedTicketTypes,
    }
  })
  ...
  function sure() { // 确定按钮
    ...
    toggleIsFiltersVisible() // 关闭图层
  }

  const isResetDisabled = useMemo(() => { // 判读是否都为默认项，都在默认项时不可重置
    return (
...
export default function Bottom(props) {
  ...
  const noChecked = useMemo(() => { // 判断综合筛选是否有输入
    return (

...
export function createAdult() { // 添加成人
  return (dispatch, getState) => {
...
export function createChild() {
  return (dispatch, getState) => {
    ...
    for (let passenger of passengers) {
      ...
      if (passenger.ticketType === 'adult') { // 判断是否有成人
...
export function removePassenger(id) { //移出乘客
  return (dispatch, getState) => {
    const { passengers } = getState()
...
export function updatePassenger(id, data, keysToBeRemoved = []) { // 输入时更新信息
  return (dispatch, getState) => {
    const { passengers } = getState()
...
export function showMenu(menu) { // 显示底部弹层
  return dispatch => {
...
export function showGenderMenu(id) { // 显示性别弹层
  return (dispatch, getState) => {
    const { passengers } = getState()
...
export function showFollowAdultMenu(id) { // 通行成人弹层
  return (dispatch, getState) => {
    const { passengers } = getState()
...
export function showTicketTypeMenu(id) { // 切换购买人类型弹层
  return (dispatch, getState) => {
    const { passengers } = getState()
